...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "list_last" {
		// query a variable and get a string which consists of all the list elements
		return t.list_last(stub, args)
	} else if function == "in_list" {
		// query a variable and get a string which consists of all the list elements
		return t.in_list(stub, args)
...
func (t *SimpleChaincode) list_last(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var A string    // Entity
	var Aval string // Asset
	var err error
	...
	Aval = decodedList[len(decodedList)-1]
	return shim.Success([]byte(Aval))
}

func (t *SimpleChaincode) in_list(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var List string    // Entity
	var Element string // Asset
	var err error
	...
	// Get the state from the ledger
	Avalbytes, err := stub.GetState(List)
	if err != nil {
		jsonResp := "{\"Error\":\"Failed to get state for " + List + "\"}"
		return shim.Error(jsonResp)
	}
	...
	if contains_str(decodedList, Element) {
